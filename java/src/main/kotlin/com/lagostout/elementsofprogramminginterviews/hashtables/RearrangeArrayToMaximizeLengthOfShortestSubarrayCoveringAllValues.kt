...
fun maximumLengthOfShortestSubarrayCoveringAllValues(values: List<Int>): Int {
    if (values.isEmpty()) return 0
    val mapOfValuesToCount = mutableMapOf<Int, Int>()
    values.forEach { value ->
        mapOfValuesToCount[value] = mapOfValuesToCount.getOrDefault(value, 0) + 1
    }
    val sortedEntries = mapOfValuesToCount.entries.stream().sorted(comparingByValue()).collect(toList())
