...
class ComputeLevenshteinDistanceInWorstCaseSpaceOfShorterStringLength : Spek({
    describe("levenshteinDistanceInWorstCaseSpaceOfShorterStringLength") {
        testCases.forEach {
            (from, to, expected) ->
            given("from: \"$from\" to: \"$to\"") {
                it("returns $expected") {
                    assertEquals(expected,
                            levenshteinDistanceInWorstCaseSpaceOfShorterStringLength(from, to))
                }
            }
        }
    }
